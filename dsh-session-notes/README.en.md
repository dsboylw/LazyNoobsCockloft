...
- **Edit popover**
  ...
  - **All-notes overview**: every session that has a note (title + note + per-row copy button); click a row to jump straight to that session
  - Bottom-bar toggle (persisted)
- **Persistent bottom bar**: when the current session has a note, a strip shows above the composer (color dot + text + copy button); clicking the text opens the popover
