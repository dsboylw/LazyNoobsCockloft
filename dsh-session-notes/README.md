...
**DSH 会话备注插件**（DSH Desktop / Web 通用，`web` 平台客户端插件）。给每个会话挂**多条**备注（1 2 3 按创建顺序编号），不用再翻会话历史找提示语。

[中文](README.md) | [English](README.en.md)

> ✅ **适配 DSH Desktop 0.8.2**（当前版本，实测正常）；历史版本 0.7.x 起兼容。
...
- **📝 头部按钮**（会话标题旁）：打开编辑弹层；当前会话有备注时按钮带**标签色圆点**（颜色跟随备注标签）
- **编辑弹层**
  - 当前会话备注编辑，防抖自动保存（400ms），显示"已保存 ✓"
  - **复制按钮**：一键复制备注全文
  - **📌 置顶开关**：写完顺手置顶，置顶时琥珀色实底高亮
  - **10 色标记**（default / amber / rose / sky / lime / purple / pink / orange / teal / blue）
  - **全部备注总览**：工作区徽标 + 标题 + 每行复制按钮；当前会话也在列（带"本会话"标记，可直接置顶，点击只关闭弹层）
  - **🔍 筛选框**：按标题 / 工作区 / 备注内容实时过滤，备注再多也不怕
  - 置顶优先排序；列表限高滚动，不撑爆屏幕
  - 底部备注栏开关（持久化）
- **底部常驻条**（始终显示在输入框上方，不随有无备注消失）
  - 彩色工作区徽章（按标签着色，文字自动黑/白保证可读）+ 会话标题（20 字）
  - 有备注：预览 + 复制按钮，点击预览打开弹层
  - 没备注：灰色提示"本会话还没有备注 — 点这里写一条"，点击直达编辑
  - **☰ 会话列表**：所有带备注会话的快捷切换菜单，随时可达不用翻顶部；置顶优先、含本会话、和弹层互斥（同时只开一个）
- **📌 置顶全链路**：列表 / 菜单 / 编辑框三处都能置顶；置顶行排最前 + 标题带 📌；悬停提示始终是完整"工作区 / 标题 — 备注"
- **长文本截断**：标题 20 字 / 工作区 10 字，超出显示 …；**复制按钮始终复制完整内容**
- **稳定性**：弹层带错误边界（PopoverGuard），弹层崩溃不影响底栏与顶部按钮；宿主 API 为完整 PATCH 语义（省略字段沿用旧值，显式空文本 = 删除）
- 数据存宿主 settings 文档（命名空间 `dsh-session-notes`），按会话 ID 持久化，重启不丢
- 清空单条备注文本 = 删除该条；上限 2000 条 / 单条 20000 字符

## 更新日志

- **0.3.1**
  - 底部栏预览跟随弹窗选中：弹窗里点选哪条备注，底部栏就显示哪条（未选中时显示最新编辑条）
- **0.3.0**
  - 新增：**多条备注**（一个会话挂 N 条提示语，告别单条塞满）+ **序号**（1 2 3 按创建顺序，稳定不重排）
  - 新增：**会话级颜色**（弹窗标题行色点，一键全组换色）+ **会话级置顶**（标题行 📌，在 ☰ 列表置顶整组）
  - 新增：**复制挑选器**（1 条直复制；多条弹挑选菜单，点哪条复制哪条，置顶优先排序）
  - 改进：底部栏精简（移除 ＋，新增统一走弹窗）；稳定性修复（locale 未注册防御、rows 缺字段防御、空草稿存活、pin 不打乱顺序）
- **0.2.0**
  - 新增：📌 置顶全链路（总览列表 / 会话菜单 / 编辑框）；🔍 全部备注筛选框；☰ 会话切换菜单；底部栏常驻（无备注可直写）；列表含本会话行
  - 改进：调色板 5 → 10 色；头部圆点与工作区徽章跟随标签色；标题 8 → 20 字、工作区 5 → 10 字；列表行只留标题（悬停看全文）；弹层加宽 480~640px、限高滚动
  - 修复：置顶清空备注（宿主 API 改为完整 PATCH 语义）；新会话弹出错误弹窗（弹层归属分家，谁打开谁渲染）；弹窗崩溃拖垮界面（PopoverGuard 错误边界）；弹窗横向滚动条（盒模型修正）
- **0.1.0** — 首发：会话备注、编辑弹层、全部备注总览、底部备注栏
...
| GET | `/notes` | 只读备注表 |
| PUT | `/notes/:id` | 新增/更新（PATCH 语义：text/color/pinned 省略字段沿用旧值；显式空文本 = 删除） |
